...
object NanzhufengMigrations {
    ...
    val MIGRATION_4_5 = object : Migration(4, 5) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL(
                "ALTER TABLE download_tasks ADD COLUMN processingStage TEXT NOT NULL DEFAULT 'NONE'",
            )
            database.execSQL(
                "ALTER TABLE download_tasks ADD COLUMN processingProgressPercent INTEGER NOT NULL DEFAULT 0",
            )
        }
    }
    ...
    val MIGRATION_5_6 = object : Migration(5, 6) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL(
                "ALTER TABLE download_tasks ADD COLUMN audioSegmentCount INTEGER NOT NULL DEFAULT 1",
            )
            database.execSQL(
                "ALTER TABLE download_history ADD COLUMN outputUrisJson TEXT NOT NULL DEFAULT '[]'",
            )
            database.execSQL(
