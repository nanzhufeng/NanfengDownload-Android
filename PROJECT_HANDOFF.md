...
- OPPO Find N5 已由 1.2.81/10281 同签名覆盖至 1.2.82/10282；首次安装时间仍为 `2026-08-21 02:04:16`，CE/DE 数据 inode 仍为 `1788131`／`1745695`，回拉设备 `base.apk` 与上述本地产物哈希一致。未启动 App、未创建/停止/重试/删除用户下载任务，因此不把它写成 OPPO 实际下载验收。
- 最终自动回归、完整证据矩阵与 checkpoint 以 `docs/verification/2026-08-23-android-v1.2.82-douyin-gallery-integrity.md` 为准；风险仍是平台页面/反爬变化，以及 OPPO 用户手动读取下载的最后一层验收。
