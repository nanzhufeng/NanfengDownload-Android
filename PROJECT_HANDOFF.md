...
3. OPPO 当前是不同签名的 `0.1.0-probe` Debug 版，无法无损覆盖为正式版；必须先确定 App 内数据迁移/放弃方案，本轮未卸载或清数据。
4. 正式签名密钥仍需用户控制的异地加密备份；本机密钥与钥匙串不能替代灾难恢复副本。

## 后续待办

下一阶段是正式版 OPPO 迁移与真实链路复验：先确定旧 Debug 数据的迁移或放弃方案，再安装正式签名版并重跑三平台成品链路。抖音 Cookie 属于外部登录条件，单独处理。
