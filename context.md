...
| 数据库 | Room 2.6.1，数据库版本 6，显式迁移 1→2→3→4→5→6 |
...
- Python 解析测试：23 项，0 失败（来自 `v1.2.3` 里程碑，本轮未重复执行）。
...
- 真实测试同时发现并修复“最高码率 6 声道 AAC 无法进入双声道 MP3 编码链”与“重试复用旧音轨缓存”两个边界：YouTube MP3 优先选择单/双声道 M4A，缓存还必须通过源体积指纹兼容检查。
...
- 历史时间线视频卡片直接显示真实媒体时长与大小；既有历史通过系统媒体 URI 读取，无需重新下载。详情弹窗同时保留完整字节数。
